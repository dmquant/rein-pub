...
表面分数：**99.4%**（95% 置信区间 98.5–100）——*这个数字没能扛住
第二位判卷者，见下文更正*。Rein 的诚实规矩同样
...
**一天之后的更正。** 同样这 398 份答案，换一个*不同模型家族*的判卷者
重判。在两位判卷者都给过分的 263 题上，同族判 0.994，异族判 0.823——
而且分歧几乎完全是单向的：异族判卷者在 158 题上给得更低，只有 1 题
更高。随机分歧应当对称；这不是随机。

真正有意思的是原因。异族判卷者归档的理由里反复出现"编造或无法核实
的具体数字"——一个精确的利率、若干精确的监管备案细节、一个漂亮得
可疑的金额区间——而同族判卷者读同样的句子，写下的是"有充分支撑的
财务指标"。两位判卷者分歧的不是文风，而是**编造出来的精确度算不算
证据**——这恰恰是整个项目存在的理由。上限就是上限，它的表现也如实
证明了这一点。

（精确的更正*数值*仍未定：第二位判卷者的服务方在第 263 题上把它掐断
了，所以那份样本是前缀而非公平抽样，第三位判卷者正在跑。方向已定，
数值未定——如实说出这一点，正是要点所在。）
...
**书房还学会了在监督下自我生长。** 一次运行的回执如今可以被蒸馏成
一份*草稿*剧本（`rein skill new`），一道确定性关卡负责把关（一句话
描述、只许引用注册过的校验器、正文必须写明自己的产出会怎样失败），
而只有操作者显式的 `promote` 才能让草稿生效。第一份生成的草稿来自
账本里自己记下的告诫：从摩根大通那次尝试蒸馏出的银行估值方法，正确
地坚持"对银行而言负债是原材料"、估值必须落在股权层面。它躺在草稿
区，校验通过，等待提拔——这正是设计的全部要点。
